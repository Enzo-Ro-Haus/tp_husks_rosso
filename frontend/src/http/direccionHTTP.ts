...
export const getAllDireccions = async (token: string | null): Promise<IDireccion[]> => {
  try {
    const response = await axios.get<IDireccion[]>(`${API_URL}/husks/v1/direccion`,
      {
        headers: {
    ...
    console.error("❌ Error al obtener Direcciones:", error.response?.data || error);
    return []; 
  }
};
